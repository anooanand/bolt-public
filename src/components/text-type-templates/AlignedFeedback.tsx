...
export function AlignedFeedback({ content, textType }: AlignedFeedbackProps) {
  ...
  // Fetch feedback data using the getNSWSelectiveFeedback function
  useEffect(() => {
    if (isOpen && content.trim().length > 50) {
      setIsLoading(true);
      setError(null);
      
      // Call the getNSWSelectiveFeedback function from openai.ts
      getNSWSelectiveFeedback(content, textType)
        .then(response => {
