/*
  # Complete Database Schema for Writing Assistant
...
CREATE POLICY IF NOT EXISTS "Users can view own writings" 
  ON writings FOR SELECT 
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own writings" 
  ON writings FOR INSERT 
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own writings" 
  ON writings FOR UPDATE 
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own writings" 
...
CREATE TRIGGER IF NOT EXISTS update_writings_updated_at
